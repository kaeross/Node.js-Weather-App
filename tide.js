...
function tide(spotID) {
	const magicSeaweedAPI = `http://magicseaweed.com/api/${key}/tide/?spot_id=${spotID}`;
	try {
		//connect to api url
		const request = http.get(magicSeaweedAPI, response => {
			...
			if (response.statusCode === 200) {
				...
				response.on('end', () => {	
					try {
						//parse data
						const tideDataArray = JSON.parse(body);
						// weatherDataArray.forEach((weatherData) => { printWeather(weatherData)});
						console.log(tideDataArray);
						console.log(tideDataArray[5].tide[1]);
					} catch (error) {
						printError(error, 70);
					}
				});
